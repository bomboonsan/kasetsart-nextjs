...
export default function ReportTableE() {
  ...
  const rows = useMemo(() => {
    ...
    for (const proj of projects) {
      ...
      const conferences = Array.isArray(proj?.conferences)
        ? proj.conferences
        : [];
      ...
      for (const c of conferences) {
        ...
        if (c?.durationStart) {
          ...
          const confEndYear = c.durationEnd
            ? new Date(c.durationEnd).getFullYear()
            : confStartYear;
  ...
  const csvData = useMemo(
    () => [
      [
        "ลำดับ",
        "ชื่อผลงานวิจัย/นำเสนอ",
        "ชื่อการประชุม/วารสาร",
        "ชื่อคณะผู้วิจัย",
        "ระดับการประชุม",
        "วัน/เดือน/ปีที่เสนอ",
      ],
