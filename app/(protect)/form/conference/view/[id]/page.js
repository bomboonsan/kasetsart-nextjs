...
const getStateLabel = (stateCode, countryCode) => {
  ...
  if (normalizedCountry) {
    const directMatch =
      typeof State.getStateByCodeAndCountry === "function"
    ...
    const fallback = states.find(
      (state) => state?.isoCode?.toUpperCase() === normalizedState
    );
...
export default function ConferenceView() {
  ...
  return (
    <div>
      <Pageheader title="ข้อมูลการประชุมวิชาการ" />
      ...
      {conference && (
        <div className="space-y-4">
          <Block>
            <div className="mb-4 flex justify-end">
              <Button
                ...
                onClick={() =>
                  router.push(
                    `/admin/form/conference/edit/${conference.documentId}`
                  )
                }
              >
                แก้ไข
            ...
            <div className="inputGroup">
              <FieldView
                label="ชื่อผลงาน (ไทย)"
                value={conference.titleTH || "-"}
              />
              <FieldView
                label="ชื่อผลงาน (อังกฤษ)"
                value={conference.titleEN || "-"}
              />
              <FieldView
                label="เกี่ยวข้องกับสิ่งแวดล้อมและความยั่งยืน"
                value={getEnvironmentalText(
                  conference.isEnvironmentallySustainable
                )}
              />
              <FieldView
                label="ชื่อการประชุมทางวิชาการ"
                value={conference.journalName || "-"}
              />
              <FieldView
                label="โครงการวิจัยที่เกี่ยวข้อง"
                value={
                  conference.projects?.[0]?.nameTH ||
                  conference.projects?.[0]?.nameEN ||
                  "-"
                }
              />
              ...
              <FieldView
                label="วัน/เดือน/ปี ที่นำเสนอ"
                value={
                  conference.durationStart && conference.durationEnd
                    ? `${formatDateToDDMMYYYY(
                        conference.durationStart
                      )} - ${formatDateToDDMMYYYY(conference.durationEnd)}`
                    : formatDateToDDMMYYYY(conference.durationStart) ||
                      formatDateToDDMMYYYY(conference.durationEnd) ||
                      "-"
                }
              />
              <FieldView
                ...
                value={
                  conference.cost
                    ? `${conference.cost.toLocaleString()} บาท จาก${getCostTypeText(
                        conference.costType
                      )}`
                    : "-"
                }
              ...
              <FieldView
                ...
                value={getPresentationWorkText(conference.presentationWork)}
              />
              <FieldView
                label="ประเภทการนำเสนอ"
                value={getPresentTypeText(conference.presentType)}
              />
              <FieldView
                label="ลักษณะของบทความ"
                value={getArticleTypeText(conference.articleType)}
              />
              <FieldView
                label="บทคัดย่อ (ไทย)"
                value={conference.abstractTH || "-"}
              />
              <FieldView
                label="บทคัดย่อ (อังกฤษ)"
                value={conference.abstractEN || "-"}
              />
              <FieldView
                label="สรุปเนื้อหา"
              ...
              <FieldView
                ...
                value={getLevelText(conference.level)}
              />
              <FieldView
                label="สถานที่"
          ...
          {conference.projects?.[0]?.partners &&
            conference.projects[0].partners.length > 0 && (
              <Block>
