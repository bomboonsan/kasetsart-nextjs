...
export default function PublicationView() {
  ...
  const scopusQuartileOptions = useMemo(
    () => toSelectOptions(listsStandardScopus),
    []
  );
  const scopusSubjectOptions = useMemo(
    () => toSelectOptions(listsStandardScopusSubset),
    []
  );
  const abdcOptions = useMemo(() => toSelectOptions(listsStandardABDC), []);
  const ajgOptions = useMemo(() => toSelectOptions(listsStandardAJG), []);
  const wosOptions = useMemo(
    () => toSelectOptions(listsStandardWebOfScience),
    []
  );
  ...
  return (
    <div>
      <Pageheader title="รายละเอียดผลงานตีพิมพ์ทางวิชาการ" />
      {loading && <div className="p-6">Loading...</div>}
      {error && (
        <div className="p-6 text-red-600">
          โหลดข้อมูลผิดพลาด: {error.message}
        </div>
      )}
      {publication && (
          ...
          <Block>
            <div className="mb-4 flex justify-end">
              <Button
                ...
                onClick={() =>
                  router.push(
                    `/form/publication/edit/${publication.documentId}`
                  )
                }
              >
                แก้ไข
            ...
            <div className="inputGroup">
              ...
              />
              <FieldView
                label="เกี่ยวข้องกับสิ่งแวดล้อมและความยั่งยืนหรือไม่"
                value={
                  publication.isEnvironmentallySustainable === "0"
                    ? "เกี่ยวข้อง"
                    : publication.isEnvironmentallySustainable === "1"
                    ? "ไม่เกี่ยวข้อง"
                    : "-"
                }
              />
              ...
              />
              <FieldView
                label="โครงการวิจัยที่เกี่ยวข้อง"
                value={publication.projects?.[0]?.nameTH || "-"}
              />
              <FieldView label="DOI" value={publication.doi || "-"} />
              <FieldView label="ISSN" value={publication.isbn || "-"} />{" "}
              {/* ใช้ isbn เก็บ ISSN ผิด */}
              <FieldView
                label="วัน/เดือน/ปี ที่ตีพิมพ์"
                value={
                  formatDateToMMYYYY(publication.durationStart) +
                  " - " +
                  formatDateToMMYYYY(publication.durationEnd)
                }
              />
              ...
              <FieldView
                label="หน้าเริ่มต้น"
                value={publication.pageStart || "-"}
              />
              <FieldView
                label="หน้าสิ้นสุด"
                value={publication.pageEnd || "-"}
              />
              <FieldView
                label="ระดับ"
                value={
              ...
              />
              <FieldView
                label="ฐานข้อมูลวารสาร"
                value={
                  publication.isJournalDatabase == "0"
                    ? "อยู่ในฐานข้อมูล"
                    : "ไม่อยู่ในฐานข้อมูล"
                }
              />
              <div
                className={
                  publication.isJournalDatabase == "0"
                    ? "bg-gray-100/60 p-4 rounded-2xl mb-5 inputGroup shadow shadow-gray-400/50"
                    : "hidden"
                }
              >
                <h2 className="text-lg font-bold">ดัชนี / มาตรฐานวารสาร</h2>
                ...
                {publication.isScopus &&
                  publication.isJournalDatabase == "0" && (
                    <FieldView
                      label="Scopus"
                      value={`${
                        scopusQuartileOptions.find(
                          (option) => option.value === publication.scopusValue
                        )?.label || "-"
                      } , ${
                        scopusSubjectOptions.find(
                          (option) => option.value === publication.scopusType
                        )?.label || "-"
                      }`}
                    />
                  )}
                ...
                {publication.isABDC && publication.isJournalDatabase == "0" && (
                  <FieldView
                    ...
                    value={`${
                      abdcOptions.find(
                        (option) => option.value === publication.abdcType
                      )?.label || "-"
                    }`}
                  />
                )}
                {publication.isAJG && publication.isJournalDatabase == "0" && (
                  <FieldView
                    ...
                    value={`${
                      ajgOptions.find(
                        (option) => option.value === publication.ajgType
                      )?.label || "-"
                    }`}
                  />
                )}
                ...
                {publication.isWOS && publication.isJournalDatabase == "0" && (
                  <FieldView
                    ...
                    value={`${
                      wosOptions.find(
                        (option) => option.value === publication.wosType
                      )?.label || "-"
                    }`}
                  />
                )}
              ...
              <FieldView
                label="ชื่อแหล่งทุน"
                value={publication.fundName || "-"}
              />
              <FieldView label="คำสำคัญ" value={publication.keywords || "-"} />
              <FieldView
                label="บทคัดย่อ (ไทย)"
                value={publication.abstractTH || "-"}
              />
              <FieldView
                label="บทคัดย่อ (อังกฤษ)"
                value={publication.abstractEN || "-"}
              />
            </div>
          </Block>
          <Block>
