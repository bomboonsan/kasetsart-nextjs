...
export default function BookView() {
  ...
  const researchTypeOptions = [
    ...
    {
      value: "ais",
      label: "AIS (Applied or Integrative / application Scholarship)",
    },
    { value: "tls", label: "TLS (Teaching and Learning Scholarship)" },
    {
      value: "nic",
      label: "NIC (Not counted as a separate Intellectual Contribution)",
    },
  ];
  ...
  return (
    <div>
      <Pageheader title="ข้อมูลหนังสือหรือตำรา" />
      ...
      {book && (
        <div className="space-y-4">
          <Block>
            <div className="mb-4 flex justify-end">
              <Button
                variant="default"
                onClick={() =>
                  router.push(`/admin/form/book/edit/${book.documentId}`)
                }
              >
                แก้ไข
              </Button>
            ...
            <div className="inputGroup">
              ...
              <FieldView
                ...
                value={book.funds?.[0]?.fundName || "-"}
              />
              <FieldView
                label="ระดับผลงาน"
                value={getLevelText(book.level) || "-"}
              />
              ...
              <FieldView
                label="Year contracted"
                value={book.yearContracted || "-"}
              />
              <FieldView label="Refereed" value={book.refereed || "-"} />
              <FieldView
                label="Research Type"
                value={getResearchTypeText(book.researchType) || "-"}
              />
              <FieldView
              ...
              {book.attachments && book.attachments.length > 0 ? (
                book.attachments.map((file) => (
                  <div key={file.documentId || file.id}>
                    <a
                      href={`${process.env.NEXT_PUBLIC_API_BASE_URL}${file.url}`}
                      target="_blank"
          ...
          {book.funds?.[0]?.partners &&
            book.funds?.[0]?.partners.length > 0 && (
              <Block>
