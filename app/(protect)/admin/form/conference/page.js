...
export default function ConferenceTable() {
  ...
  const [deleteConference, { loading: deleteLoading }] =
    useMutation(DELETE_CONFERENCE);
  ...
  let conferences = rawConferences;
  ...
  if (roleName === "Admin" && myDeptId) {
    conferences = conferences.filter((conference) => {
      ...
      if (!conference?.projects || conference.projects.length === 0)
        return false;
  ...
  const canGoNextPage = rawConferences.length === PAGE_SIZE;
  ...
  return (
    <div>
      <Pageheader
        title="จัดการข้อมูลการประชุมวิชาการ"
        btnName="เพิ่มข้อมูลการประชุมวิชาการ"
        btnLink="/form/create/conference"
      />
      ...
      <div className="text-sm text-gray-600 mb-4">
        แสดง {conferences.length} รายการ (หน้า {page})
      </div>
      ...
      <div className="bg-white rounded shadow overflow-hidden">
        <Table>
          ...
          <TableBody>
            ...
            {error && (
              <TableRow>
                <TableCell colSpan={8} className="p-6 text-center text-red-600">
            ...
            {conferences.map((c) => (
              <TableRow key={c.documentId}>
                ...
                <TableCell className={"px-5 md:max-w-64 whitespace-normal"}>
                  {getLocationLabel(c.city, c.state, c.country)}
                </TableCell>
                <TableCell className={"px-5"}>
                  {c.durationStart &&
                  c.durationEnd &&
                  c.durationStart !== c.durationEnd
                    ? `${formatDateToDDMMYYYY(
                        c.durationStart
                      )} - ${formatDateToDDMMYYYY(c.durationEnd)}`
                ...
                <TableCell className="text-right px-5">
                  <div className="flex justify-end gap-3">
                    <a
                      ...
                      href={`/form/conference/view/${c.documentId}`}
                    >
                      ดู
                    </a>
                    <a
                      className="text-green-600"
                      href={`/form/conference/edit/${c.documentId}`}
                    >
                      แก้ไข
                    </a>
                    <Button
