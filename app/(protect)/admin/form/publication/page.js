...
export default function PublicationTable() {
  ...
  const [deletePublication, { loading: deleteLoading }] =
  ...
  let publications = rawPublications;
  ...
  const canGoNextPage = rawPublications.length === PAGE_SIZE;
  ...
  return (
    <div>
      ...
      <div className="text-sm text-gray-600 mb-4">
        แสดง {publications.length} รายการ (หน้า {page})
      </div>
            ...
            {publications.map((p) => (
              <TableRow key={p.documentId}>
                ...
                <TableCell className={"px-5"}>
                  ...
                  {formatDateToMMYYYY(p.durationEnd) || "-"}
                </TableCell>
                <TableCell className={"px-5"}>
                  {getLevelText(p.level)}
                </TableCell>
                <TableCell className={"px-5"}>
                  {p.isJournalDatabase == "0"
                    ? "อยู่ในฐานข้อมูล"
                    : "ไม่อยู่ในฐานข้อมูล"}
                </TableCell>
                <TableCell className={"px-5"}>
                ...
                <TableCell className="text-right px-5">
                  <div className="flex justify-end gap-3">
                    <a
                      ...
                      href={`/form/publication/view/${p.documentId}`}
                    >
                      ดู
                    </a>
                    <a
                      className="text-green-600"
                      href={`/form/publication/edit/${p.documentId}`}
                    >
                      แก้ไข
                    </a>
                    <Button
      ...
      <TablePagination
        page={page}
        pageSize={PAGE_SIZE}
        itemsCount={publications.length}
